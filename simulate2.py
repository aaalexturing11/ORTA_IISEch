...
def _target_speed(seg: pd.Series, drv: DriverProfile, env: dict) -> float:
    ...
    return float(np.clip(base, 25.0, 110.0))
...
def simulate_trip(
    ...
    for i, seg in route_df.iterrows():
        ...
        target_kmh = _target_speed(seg, drv, env)
        ...
        row = {
            ...
            "precip_mmph": env["precip_mmph"],
            "road_wet": env["road_wet"],
...
def _load_route(route_slug: str | None) -> tuple[dict, pd.DataFrame, str]:
    """Load route by folder slug. If None, picks the most recently modified route."""
    if route_slug is None:
        candidates = sorted(
            ROUTES_DIR.glob("*/route.json"), key=lambda p: p.stat().st_mtime
        )
        if not candidates:
            raise SystemExit(
                f"No routes in {ROUTES_DIR}/ \u2014 generate one with datosDeRuta.py first."
            )
        route_slug = candidates[-1].parent.name

    route_dir = ROUTES_DIR / route_slug
    meta_path = route_dir / "route.json"
    parquet = route_dir / "route.parquet"
    csv = route_dir / "route.csv"
    if not meta_path.exists():
        raise SystemExit(f"No route metadata at {meta_path}")
    ...
    if parquet.exists():
        ...
        raise SystemExit(f"No segment data for route {route_slug}")
    return meta, df, route_slug
...
def run_simulation(
    ...
    route_meta, route_df, route_slug = _load_route(route_slug)
    ...
    truck_slug = slugify(truck.model_name)
    sim_dir = ROUTES_DIR / route_slug / "sims" / truck_slug
    sim_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S")
    run_id = f"{stamp}_{uuid.uuid4().hex[:6]}"
    parquet_path = sim_dir / f"sim_{run_id}_trips.parquet"
    json_path = sim_dir / f"sim_{run_id}_drivers.json"
    try:
...
def _cli() -> None:
    ...
    p.add_argument("route_slug", nargs="?", default=None, help="Route folder slug (default: latest)")
    p.add_argument(
        "--truck-model",
        default="14l-weichai",
